...
-- ===== 버그 수정 (2026-09-06): 성경책 순서 맞추기 "기록 저장에 실패했어요" =====
-- submit_book_game_score()가 points_ledger에 action_type='book_game_ot'/'book_game_nt'를
-- 적립하려 하는데, points_ledger_action_type_check CHECK 제약에 이 값들이 빠져 있어서
-- insert가 제약 위반으로 터지고 함수 전체가 롤백됨 → 로그인 사용자는 점수/달란트가
-- 한 번도 저장된 적이 없었음(게임 추가된 2026-09-02부터 계속). 허용 목록에 추가한다.
alter table points_ledger drop constraint if exists points_ledger_action_type_check;
alter table points_ledger add constraint points_ledger_action_type_check
...
-- ===== 첨부 이미지 + 본문 링크 (2026-09-06) =====
-- 하루인사/감사노트/기도제목/자유게시판 글·댓글에 이미지 여러 장 첨부. 이미지는 Storage 버킷
-- 'post-images'(공개 읽기)에 올리고 공개 URL 배열을 image_urls(text[])에 저장한다. 본문의
-- http(s) 링크 자동연결은 클라이언트(post-media.js)에서만 처리한다.
-- 실행용 스니펫: sql-migrations/2026-09-06-post-images-and-links.sql
...
-- ===== 같은 성경 찾기 (카드 뒤집기 짝 맞추기) — 2026-09-06 =====
...
-- ===== 달란트 상점(교환소) — 2026-09-06 =====
-- 실행용: sql-migrations/2026-09-06-talent-shop.sql
...
declare v_item shop_items; v_balance integer; v_order_id bigint;
begin
...
language sql security definer set search_path = public as $$
  ...
  from shop_orders o where o.user_id = auth.uid()
...
-- ===== 달란트 상점 "상품 추천함" — 2026-09-06 =====
-- 실행용: sql-migrations/2026-09-06-shop-suggestions.sql
...
begin
  ...
  select count(*) into v_today_count from shop_suggestions
  where user_id = auth.uid() and created_at >= (now() at time zone 'Asia/Seoul')::date;
...
language sql security definer set search_path = public as $$
  ...
  from shop_suggestions s where s.user_id = auth.uid()
...
begin
  ...
  update shop_suggestions set status = p_status, admin_note = nullif(trim(p_note), '') where id = p_id;
  return true;
end;
$$;

-- ===== 교사 오이코스 회식비 지원 — 2026-09-06 =====
-- 실행용: sql-migrations/2026-09-06-oikos-expense.sql
